    ...
    std::vector<HRilCBConfigInfo> &cellBroadcastInfo, const OHOS::HDI::Ril::V1_1::CBConfigInfo &broadcastInfo)
{
    ...
    for (auto mid : mids) {
        ...
        info.startOfServiceId = strtol(startMid);
        info.endOfServiceId = strtol(endMid);
        info.startOfCodeScheme = strtol(startDcs);
        info.endOfCodeScheme = strtol(endDcs);
...
bool HRilSms::CreateCdmaMessageInfo(HRilCdmaSmsMessageInfo &cdmaSmsInfo, const std::string &pdu)
{
    ...
    cdmaSmsInfo.serviceId = strtol(pdu.substr(0, INT_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.isExist = strtol(pdu.substr(INT_LEN + BYTE_LEN * NUM_3, BYTE_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.type = strtol(pdu.substr(INT_LEN + INT_LEN, INT_LEN), nullptr, HEXADECIMAL);
    int32_t index = INT_LEN * NUM_3;
    // adress
    cdmaSmsInfo.address.digitMode = strtol(pdu.substr(index, BYTE_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.address.mode = strtol(pdu.substr(index + BYTE_LEN, BYTE_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.address.type = strtol(pdu.substr(index + BYTE_LEN * NUM_2, BYTE_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.address.plan = strtol(pdu.substr(index + BYTE_LEN * NUM_3, BYTE_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.address.number = strtol(pdu.substr(index + BYTE_LEN * NUM_4, BYTE_LEN), nullptr, HEXADECIMAL);
    ...
    // subAdress
    cdmaSmsInfo.subAddress.type = strtol(pdu.substr(index, BYTE_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.subAddress.odd = strtol(pdu.substr(index + BYTE_LEN, BYTE_LEN), nullptr, HEXADECIMAL);
    cdmaSmsInfo.subAddress.number = strtol(pdu.substr(index + BYTE_LEN * NUM_2, BYTE_LEN), nullptr, HEXADECIMAL);
...
bool HRilSms::CheckCdmaPduLength(HRilCdmaSmsMessageInfo &cdmaSmsInfo, const std::string &pdu)
{
    ...
    cdmaSmsInfo.address.number = strtol(pdu.substr(index - BYTE_LEN, BYTE_LEN), nullptr, HEXADECIMAL);
    ...
    // subAdress
    cdmaSmsInfo.subAddress.number = strtol(pdu.substr(index - BYTE_LEN, BYTE_LEN), nullptr, HEXADECIMAL);
    ...
    // bearer Data
    cdmaSmsInfo.size = strtol(pdu.substr(index - BYTE_LEN, BYTE_LEN), nullptr, HEXADECIMAL);

...
typedef struct {
    int32_t callId; /* Call ID */
    int32_t status; /* Indicates the call status of the corresponding callid.
                     * 0: Call originate(MO).
                     ...
                     * 7: Hold
                     * 8: Retrieve */
    int32_t voiceDomain; /* Call domain for voice call.
                          * 0: CS domain voice call.
                          * 1: IMS domain voice call. */
} HRilCallStatusInfo;

typedef struct {
    int32_t callId; /* Call ID, Value range: 1 ~ 7 */
    int32_t duration; /* Call duration, unit: s. */
    int32_t noCliCause; /* No caller number reason value.
                         * 0: The number is not available.
                         * 1: The user refused to provide a number.
                         * 2: Affected by other services.
                         * 3: Pay phone. */
    int32_t ccCause; /* The value of the reason for the call error.
...
typedef struct {
    ...
    int32_t type; /* Number address type.
                   * 129: Normal number.
                   * 145: International number (starting with "+"). */
    int32_t businessClass; /* Business category, the value is a combination of the following business categories,
